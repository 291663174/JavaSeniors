 ...
 *  4.jdk提供的4种元注解
 *     元注解：对现有的注解进行解释说明的注解
 *    Retention:指定所修饰的Annotation的生命周期：SOURSE\CLASS(默认行为，在class中有效)\RUNTIME
 *              只有声明为RUNTIME生命周期的注解，才能通过反射获取
 *    Target:用于指定被修饰的Annotation能用于修饰哪些程序元素
 *   **********出现频率较低*********
 *    Documented：表示所修饰的注解在被javadoc解析时，保留下来
 *    Inherited：被它修饰的注解具有继承性，子类自动继承该注解
 *   5.通过反射获取注解信息
 *   6.jdk 8 中注解得新特性：可重复注解、类型注解
 *      1.可重复注解：① 在MyAnnotation上声明一个@Repeatable,成员值为MyAnnotations.class
 *                  ②  MyAnnotation的Target和Retention等元注解需要与MyAnnotations相同
 *      2.类型注解
 *          ElementType.TYPE_PARAMETER 表示该注解能写在类型变量的声明语句中（如：泛型声明）。
 *          ElementType.TYPE_USE 表示该注解能写在使用类型的任何语句中。
 * @date 2020-09-30 23:05
 **/
...
/**
 *  下面@Target(...TYPE_PARAMETER)解决的报错
 **/
class Generic<@MyAnnotation T>{
    /**
     *  下面@Target(...TYPE_USE)解决的报错
     **/
