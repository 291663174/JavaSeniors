 ...
 *              LinkedHashMap:保证在遍历map元素的，可以按照添加的顺序实现遍历。
 ...
 *      ....可能已经执行很多次put....
 *      map.put(key1,value1)
 *      首先调用key1所在类的hashCode()计算key1的哈希值，此哈希值警告某种算法计算以后，得到在Entry数组中的存放位置
 ...
 *          在不断的添加过程中：会涉及到扩容问题，当超出临界值（且要存放的位置非空）时，扩容为原本的2倍
 *          默认的扩容方式：扩容为原来容量的2倍，并将原有的数据复制过来。
 *          jdk8相较于jdk7在底层实现方面的不同。
 *              1. new HashMap():底层没有创建一个长度为16的数组
 *              2. jdk 8 底层的数组是:Nodep[],而非Entry[]
 *              3. 首次调用put()方法时，底层创建长度为16的数组
 *              4. jdk7底层结构只有：数组+链表。jdk8底层结构：数组+链表+红黑树
 *                  当数组的某一个索引位置上的元素以链表形式存在的数据个数 > 8 且当前数组的长度 > 64时，此时此索引
 *                  位置上的所有数据改为使用红黑树存储。
 *    总结：常用方法：
...
public class MapTest {
     ...
     *  boolean containsKey(Object key)_:是否包含指定的value
